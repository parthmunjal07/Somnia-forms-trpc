...
function FogAnimation({ isDestabilizing = false }: { isDestabilizing?: boolean }) {
  ...
  const destabilizingRef = useRef(isDestabilizing);

  useEffect(() => {
    destabilizingRef.current = isDestabilizing;
  }, [isDestabilizing]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const p of particles) {
        ...
        const isDest = destabilizingRef.current;
        const color = isDest
          ? `rgba(220,20,20,${p.opacity * 3})`
          : p.gold
